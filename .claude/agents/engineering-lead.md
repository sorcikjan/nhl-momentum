...
- **ProCyclingStats** — has run the same point-scale calculation for decades across millions of race results. Their key architectural lesson: immutability of historical computed values. Once a rider's points for a race are calculated with a given formula, they don't get recalculated retroactively when the formula changes — the change applies going forward, and older values stay. We need the same discipline for PPM: `player_metric_snapshots` rows are immutable records of what the metric was on that date. If the formula changes in v1.9, old snapshots don't get rewritten. Their data model is also instructive — rider + race + result as the three core entities, with everything else (team, year, stage type) as dimensions. Our equivalent is player + game + stat_row. Any feature touching leaderboards or career stats should be modeled as a query over that base triple, not a new table.

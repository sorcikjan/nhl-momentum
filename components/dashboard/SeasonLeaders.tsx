    ...
    <div className="rounded-xl border p-4 flex flex-col" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-xs px-2 py-0.5 rounded"
          style={{ background: 'rgba(148,163,184,0.1)', color: 'var(--silver)' }}>
          Full Season
        </span>
      </div>
      <p className="text-xs mb-2" style={{ color: 'var(--text)' }}>
        Top skaters by season PPM — consistent performers across the whole season.
      </p>
      ...
      <div className="flex flex-col gap-2 flex-1">
        {visible.map((p, i) => {
          ...
          const ppm = p.season_ppm?.toFixed(4) ?? '—';
          return (
            <Link key={p.player_id}
              ...
              <span className="text-xs w-5 text-center font-mono font-bold" style={{ color: 'var(--silver)' }}>
                {i + 1}
              </span>
              ...
              <div className="text-right">
                <div className="text-sm font-mono font-semibold" style={{ color: 'var(--silver)' }}>{ppm}</div>

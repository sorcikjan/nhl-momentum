  ...
  return (
    <div className="rounded-xl border p-4 flex flex-col" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'var(--neon-glow)', color: 'var(--neon)', border: '1px solid var(--neon)' }}>
          Last 5 Games
        </span>
      </div>
      <p className="text-xs mb-2" style={{ color: 'var(--text)' }}>
        Top skaters by momentum PPM over their last 5 games — the hottest players in the league right now.
      </p>
      ...
      <div className="flex flex-col gap-2 flex-1">
        {players.map((p, idx) => {
          ...
          const pct = p.season_ppm > 0
            ...
            : 0;
          const barPct = Math.min(100, (p.momentum_ppm / 0.06) * 100);
          ...
          return (
            <Link key={p.player_id}
              ...
              <span className="text-xs font-mono w-4 text-center flex-shrink-0" style={{ color: 'var(--text)', opacity: 0.5 }}>
                {idx + 1}
              </span>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-mono font-semibold ml-2 flex-shrink-0" style={{ color: 'var(--neon)' }}>
                    {p.momentum_ppm?.toFixed(4)}
                  </span>
                </div>
                <div className="w-full h-1.5 rounded-full" style={{ background: 'var(--border)' }}>
                  <div className="h-1.5 rounded-full transition-all" style={{ width: `${barPct}%`, background: 'var(--neon)' }} />
                </div>
                <div className="flex justify-between mt-1">
                  ...
                  {pct !== 0 && (
                    <span className="text-xs font-mono" style={{ color: pct > 0 ? 'var(--neon)' : 'var(--text)' }}>
                      {pct > 0 ? '+' : ''}{pct}% vs avg
                    </span>
                  )}
                </div>
      ...
      <div className="mt-3 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
        <div className="flex items-center justify-between">
          <span className="text-xs" style={{ color: 'var(--text)' }}>Top 10 by momentum PPM</span>
          <Link href="/rankings" className="text-xs font-medium hover:underline" style={{ color: 'var(--neon)' }}>
            Full rankings →
          </Link>
        </div>
      </div>

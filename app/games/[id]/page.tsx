    ...
    <div className="max-w-5xl mx-auto pb-20 md:pb-0">
      ...
      <div className="rounded-xl border p-6 mb-4" style={{ background: 'var(--bg-card)', borderColor: isLive ? 'var(--red)' : 'var(--border)' }}>
        ...
        <div className="flex items-center justify-between gap-4">
          {/* Away team */}
          <Link href={awayId ? `/teams/${awayId}` : '#'} className="flex flex-col items-center gap-2 flex-1 hover:opacity-80">
          ...
          {/* Home team */}
          <Link href={homeId ? `/teams/${homeId}` : '#'} className="flex flex-col items-center gap-2 flex-1 hover:opacity-80">
...
function LineupCard({
  abbrev, logo, skaters, goalie, isLive, teamId,
}: {
  abbrev: string;
  logo: string;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return (
    <div className="rounded-xl border overflow-hidden" style={{ borderColor: 'var(--border)' }}>
      <div className="px-4 py-3 border-b flex items-center gap-2"
        ...
        <Link href={`/teams/${teamId}`} className="text-sm font-semibold hover:opacity-80" style={{ color: 'var(--text-bright)' }}>
          {abbrev}
        </Link>

...
import { MODEL_REGISTRY, type ModelResult, type TeamSnap } from '@/lib/prediction-models';
...
// Model formula registry is shared with the daily ingest pipeline.
// All model functions live in lib/prediction-models.ts.
const MODEL_FORMULAS: Record<string, (homeSnap: TeamSnap, awaySnap: TeamSnap) => ModelResult> = MODEL_REGISTRY;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!MODEL_FORMULAS[model_version]) {
      return NextResponse.json({
        data: null,
        error: `No formula registered for ${model_version}. Add it to lib/prediction-models.ts`,
      }, { status: 400 });
    }
    ...
    for (const [gameId, { home, away }] of byGame) {
      ...
      const result = formula(homeSnap, awaySnap);
